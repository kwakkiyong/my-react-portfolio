...
2. **자동 포맷팅 설정 (VS Code)**
   VS Code 설정 파일 (`.vscode/settings.json`)에 다음을 추가:
   ```json
   {
...
1. 프로젝트 구조를 참고하여 필요한 컴포넌트와 페이지를 생성하세요
2. `tailwind.config.js`에서 커스텀 색상과 스타일을 정의하세요
3. `src/types`에 프로젝트에 필요한 타입을 정의하세요
4. 데이터가 필요한 경우 `src/data` 폴더에 정적 데이터를 추가하세요
